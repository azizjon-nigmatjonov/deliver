...
export default function DiscountsTable() {
  ...
  const updateIikoBranches = () => {
    getCRMDiscounts("poster", { page: currentPage, limit }).then(() => {
  ...
  const getItems = useCallback(
    (page) => {
      setLoader(true);
      getIntegrationDiscounts({ limit, page, search })
        .then((res) => {
          setItems({
            count: res?.count,
            data: res?.discounts,
      ...
      },
      {
        ...
        render: (record) => (
          <Tag
            className="p-1"
            color={
              record.discount_data?.poster_discount_id ? "primary" : "warning"
            }
            lightMode={true}
          >
            {record.discount_data?.poster_discount_id
              ? t("connected")
              : t("not.connected")}
          </Tag>
        ),
